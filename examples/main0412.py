...
class YelpBusinessDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, tiny_set, tiny_size, location, sep=',', engine='c', header='infer'):
        ...
        data = data.to_numpy()
        # self.text = data[:60000, 5:].astype(np.float32)
        if tiny_set:
...
def get_model(name, dataset, add_text):
    ...
    if name == 'fm':
        ...
        return DeepFactorizationMachineModel(field_dims1, field_dims2, embed_dim=4, mlp_dims=(4,1,1), dropout=0.4, add_text=add_text)
    elif name == 'dcn':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--domain_similar', type=bool, default=True)
    parser.add_argument('--add_text', type=bool, default=True)
    parser.add_argument('--tiny_set', type=bool, default=True)
    parser.add_argument('--tiny_size', type=int, default=30000)
