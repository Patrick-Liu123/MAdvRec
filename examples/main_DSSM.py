...
class YelpBusinessDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_path, tiny_set, tiny_size, location, sep=',', engine='c', header='infer'):
        ...
        data = data.to_numpy()
        if tiny_set:
            self.text = np.concatenate([data[:int(tiny_size/2), 5:].astype(np.float32), data[int(120000-tiny_size/2):, 5:].astype(np.float32)], axis=0)
        else:
        ...
        # print(data1)
        if tiny_set:
            data = np.concatenate([data1[:int(tiny_size/2), :].astype(np.int), data1[int(120000-tiny_size/2):, :].astype(np.int)], axis=0)
        else:
            data = data1
...
def get_model(name, dataset, add_text):
    ...
    if name == 'fm':
        ...
        return DeepFactorizationMachineModel(field_dims1, field_dims2, embed_dim=4, mlp_dims=(4,1,1), dropout=0.5, add_text=add_text)
    elif name == 'dcn':
...
def train(model, optimizer, data_loader, criterion, device, epoch, log_interval=100, adv=False, domain_similar=False, add_text=False):
    ...
    for i, (fields, target) in enumerate(tk0):
        ...
        optimizer.zero_grad()
        _, loss, loss_sim = model(f_b, f_u, f_t, True, epoch, criterion, target.float(),adv=adv,domain_similar=domain_similar, add_text=add_text, dssm=True)
        # loss_ = criterion(y, target.float())
        # loss = loss_ + 0.1 * loss
        # model.zero_grad()
        loss = loss + 0.3 * loss_sim
        loss.backward()
        optimizer.step()
...
def test(model, data_loader, device, add_text=False):
    ...
    with torch.no_grad():
        for fields, target in tqdm.tqdm(data_loader, smoothing=0, mininterval=1.0):
            ...
            f_t = fields[2]
            f_b, f_u, f_t, target = f_b.to(device), f_u.to(device), f_t.to(device), target.to(device)
            y,_,_ = model(f_b, f_u, f_t, False, epoch=-1, loss_fct=None,label=target.float(), add_text=add_text, dssm=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_text', type=bool, default=False)
    parser.add_argument('--tiny_set', type=bool, default=True)
    parser.add_argument('--tiny_size', type=int, default=60000)
    parser.add_argument('--location', type=bool, default=False)
