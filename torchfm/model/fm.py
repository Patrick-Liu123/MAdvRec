...
class FactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, field_dims1, field_dims2, embed_dim, add_text=False):
        super().__init__()
        # self.embedding = nn.ModuleList(
        #     [FeaturesEmbedding(field_dims1, embed_dim),
        #      FeaturesEmbedding(field_dims2, embed_dim)
        #      ])
    ...
    def forward(self, x_b, x_u, text, train, epoch, loss_fct, label, adv=False, domain_similar=False, add_text=False, dssm=False):
        ...
        if dssm:
            b = self.linear1(x_b) + self.fm(self.embedding1(x_b))
            u = self.linear2(x_u) + self.fm(self.embedding2(x_u))
            # b = self.embedding1(x_b)
            # u = self.embedding2(x_u)
            sim_loss_fct = torch.nn.CosineEmbeddingLoss()
            loss_dssm = sim_loss_fct(b.squeeze(1), u.squeeze(1), label)
            mul = torch.mul(b.squeeze(1), u.squeeze(1))
            mul = torch.sum(mul,1)
            out = torch.sigmoid(mul)
            return out, loss_dssm, 0

        # x_out = self.linear(x) + self.fm(self.embedding(x))
        # emb1 = self.embedding[0]
        # emb2 = self.embedding[1]
        x_bus = self.linear1(x_b) + self.fm(self.embedding1(x_b))
        x_user = self.linear2(x_u) + self.fm(self.embedding2(x_u))
        # print(x_b)
        # print(x_u)
        ...
        if train:
            if epoch > 0 and adv:
                ...
                if domain_similar:
                    ...
                    y = torch.ones(len(x_bus)).to(device)
                    loss_sim = sim_loss_fct(x_bus, x_user, y)
                else:
                    loss_sim = 0
                ...
                x_b = self.linear1(x_b) + self.fm(embed_b)
                x_u = self.linear2(x_u) + self.fm(embed_u)

                if add_text:
                    x_out = torch.cat([x_b, x_u, text_embed], dim=1)
                else:
                    x_out = torch.cat([x_b, x_u], dim=1)
                # print(x_out.shape)
                x_out = self.output(x_out)
                out = torch.sigmoid(x_out.squeeze(1))
